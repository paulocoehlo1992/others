...
master_sub["new"]=master_sub["Variant Name"]+" " +master_sub['Fuel Type'] +" "+ master_sub['Cubic Capacity']
master_sub['make_model']=master_sub['Make Name'] +" "+master_sub['Model Name']
master_sub['new']=master_sub['new'].astype("str")
master_sub['make_model']=master_sub['make_model'].astype('str')
master_sub_make_model=master_sub.drop_duplicates(subset=['make_model'])
...
def search_restrict_make_model(text_array,json_array):
    restriced_result=[]
    for json in json_array:
        z=json['concat'].lower()
        if all(x in z for x in text_array):
            restriced_result.append(json)
    return sorted(restriced_result , key=lambda elem: "%s %s " % (elem['make'],elem['model']))
...
#function for returning make and model 

@app.route('/make_model/<string:text>',methods=['GET','HEAD'])
def make_model_scorer(text):
	c=[]
	for found,score,matchrow in p.extract(text,master_sub_make_model['make_model'],limit=30,scorer=fuzzywuzzy.fuzz.partial_token_sort_ratio):
		if score>60:
			a={}
			a['score']=int(score)
			a['text']=text
			a['make']=str(master_sub_make_model.loc[matchrow,'Make Name'])
			a['model']=str(master_sub_make_model.loc[matchrow,'Model Name'])
			a['concat']=str(master_sub_make_model.loc[matchrow,'Make Name']) +" " + str(master_sub_make_model.loc[matchrow,'Model Name'])
			c.append(a)
	output = sorted(c, key=itemgetter('score'),reverse=True)
	w=search_restrict_make_model(breakStringlower(text),output)
	if len(w)==0:
		return(jsonify(output))
	else:
		return(jsonify(w))

#function for returning the varaint  based on make and model 

@app.route('/vehicle',methods=['POST'])
def scorer():
	req_data = request.get_json()
	Make = req_data['make']
	Model = req_data['model'] #if key doesn't exist, returns a 400, bad request error
	text = 'civic'
	print(Make,Model)
	master_sub_var=master_sub.loc[(master_sub["Make Name"]==Make) &(master_sub['Model Name']==Model)]

	b=[]
	for found, score, matchrow in p.extract(reduce_string(text), master_sub_var["new"], limit=2,scorer=fuzzywuzzy.fuzz.partial_token_sort_ratio):# processor=lambda x:x):
	    if score >40:
	        a={}
	        a["score"]=int(score)
	        a["text"]=text
	...
	return jsonify(json.dumps(json.loads(master_sub_var.to_json(orient='records')), indent=2))
@app.before_request
def before_request():
  g.start = time.time()
...
if __name__ == '__main__':
    ...
    app.run(host="10.206.226.53",port=5001)
